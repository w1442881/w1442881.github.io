(() => {
  ...
  var require_worker = __commonJS({
    "node_modules/graphology-layout-force/worker.js"(exports, module) {
      ...
      function ForceSupervisor2(graph, params) {
        if (!isGraph2(graph))
          throw new Error(
      ...
      ForceSupervisor2.prototype.isRunning = function() {
        return this.running;
      };
      ForceSupervisor2.prototype.runFrame = function() {
      ...
      ForceSupervisor2.prototype.stop = function() {
        this.running = false;
      ...
      ForceSupervisor2.prototype.start = function() {
        if (this.killed)
      ...
      ForceSupervisor2.prototype.kill = function() {
        this.stop();
        delete this.nodeStates;
        this.killed = true;
      };
      module.exports = ForceSupervisor2;
    }
  });
...
`
  ...
  // src/client/grapher.ts
  var Grapher = class {
    _inConnectMode = false;
    _inNodeDeleteMode = false;
    _inEditContentMode = false;
    graph;
    renderer;
    get connectMode() {
      return this._inConnectMode;
    }
    set connectMode(connected) {
      this._inConnectMode = connected;
      if (connected) {
        this._inNodeDeleteMode = false;
        this._inEditContentMode = false;
      }
    }
    get contentMode() {
      return this._inEditContentMode;
    }
    set contentMode(content) {
      this._inEditContentMode = content;
      if (content) {
        this._inNodeDeleteMode = false;
        this._inConnectMode = false;
      }
    }
    get deleteNodeMode() {
      return this._inNodeDeleteMode;
    }
    set deleteNodeMode(deleteNodeMode) {
      this._inNodeDeleteMode = deleteNodeMode;
      if (deleteNodeMode) {
        this._inConnectMode = false;
        this._inEditContentMode = false;
      }
    ...
    selectedNodes = [];
    constructor() {
      this.graph = new import_graphology.default();
      ...
      this.configureInitialDragEvents();
    }
    addNode() {
      const id = v4_default();
      var node = this.graph.addNode(id, {
      ...
      this.graph.setNodeAttribute(node, "x", 100 * Math.random());
      this.graph.setNodeAttribute(node, "y", 100 * Math.random());
      this.graph.setNodeAttribute(node, "labelColor", "#8a8a4c");
      const layout = new import_worker.default(this.graph, {
    ...
    connectSelectedNodes() {
      ...
      for (let i = 0; i < this.selectedNodes.length - 1; i++) {
        if (!this.graph.hasEdge(this.selectedNodes[i], this.selectedNodes[i + 1]))
          this.graph.addEdge(
            this.selectedNodes[i],
            this.selectedNodes[i + 1],
            { type: "arrow", label: "s", size: 3, forceLabel: true }
          );
      }
      this.resetNodes();
    }
    ...
    resetNodes() {
      this.selectedNodes.forEach((x) => {
        this.graph.setNodeAttribute(x, "color", "#db8e09");
      });
      this.selectedNodes = [];
      const layout = new import_worker.default(this.graph, {
        isNodeFixed: (_, attr) => attr.highlighted
    ...
    configureInitialDragEvents() {
      ...
      this.renderer.on("downNode", (e) => {
        ...
        if (this._inConnectMode) {
          const index = this.selectedNodes.findIndex((x) => x === draggedNode);
          if (index == -1) {
            this.graph.setNodeAttribute(draggedNode, "color", "#123456");
            this.selectedNodes.push(draggedNode);
          }
        }
      ...
      const handleUp = () => {
        if (draggedNode) {
          ...
          if (this._inNodeDeleteMode) {
            this.graph.dropNode(draggedNode);
            const index = this.selectedNodes.findIndex((x) => x === draggedNode);
  ...
  // src/client/graph-editor.ts
  var grapher = new Grapher();
  var addNodeButton = document.getElementById("add-node");
  var connectNodesButton = document.getElementById("connect-nodes");
  var disconnectNodesButton = document.getElementById("disconnect-nodes");
  var toggleConnectButton = document.getElementById("connect-mode");
  var toggleContentButton = document.getElementById("content-mode");
  var toggleDeleteNodeButton = document.getElementById("delete-node");
  addNodeButton.addEventListener("click", () => {
    grapher.addNode();
  });
  connectNodesButton.addEventListener("click", (event) => {
    event.stopPropagation();
    grapher.connectSelectedNodes();
  });
  disconnectNodesButton.addEventListener("click", (event) => {
    event.stopPropagation();
    grapher.disconnectSelectedNodes();
  });
  toggleConnectButton.addEventListener("click", () => {
    grapher.connectMode = !grapher.connectMode;
    toggleContentButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
    toggleDeleteNodeButton.innerHTML = "<i class='fa fa-trash'></i> <i class='fa fa-toggle-off'></i>";
    grapher.resetNodes();
    if (grapher.connectMode) {
      toggleConnectButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-on'></i>";
      connectNodesButton.disabled = false;
      disconnectNodesButton.disabled = false;
    } else {
      toggleConnectButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
      connectNodesButton.disabled = true;
      disconnectNodesButton.disabled = true;
    }
  });
  toggleContentButton.addEventListener("click", () => {
    grapher.contentMode = !grapher.contentMode;
    toggleConnectButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
    toggleDeleteNodeButton.innerHTML = "<i class='fa fa-trash'></i> <i class='fa fa-toggle-off'></i>";
    grapher.resetNodes();
    if (grapher.contentMode)
      toggleContentButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-on'></i>";
    else
      toggleContentButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
  });
  toggleDeleteNodeButton.addEventListener("click", () => {
    grapher.deleteNodeMode = !grapher.deleteNodeMode;
    toggleConnectButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
    toggleContentButton.innerHTML = "<i class='fa fa-edit'></i> <i class='fa fa-toggle-off'></i>";
    grapher.resetNodes();
    if (grapher.deleteNodeMode)
      toggleDeleteNodeButton.innerHTML = "<i class='fa fa-trash'></i> <i class='fa fa-toggle-on'></i>";
    else
      toggleDeleteNodeButton.innerHTML = "<i class='fa fa-trash'></i> <i class='fa fa-toggle-off'></i>";
  });
})();
